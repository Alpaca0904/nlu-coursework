...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            ...
            net_in = np.dot(self.V, x_vector) + np.dot(self.U, s[t - 1])
            s[t] = sigmoid(net_in)
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            d_t_vector = make_onehot(d[t], self.out_vocab_size)
            delta_out_t = (d_t_vector - y[t]) * net_out_grad[t]
            sum_deltaW += np.outer(delta_out_t, s[t])
            delta_in_t = (np.dot(self.W.T, delta_out_t)) * net_in_grad[t]
            x_vector = make_onehot(x[t], self.vocab_size)
            sum_deltaV += np.outer(delta_in_t, x_vector)
            sum_deltaU += np.outer(delta_in_t, s[t-1])
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        d_t_vector = make_onehot(d[0], self.out_vocab_size)
        delta_out_t = (d_t_vector - y[-1]) * net_out_grad[-1]
        sum_deltaW = np.outer(delta_out_t, s[-2])
        delta_in_t = (np.dot(self.W.T, delta_out_t)) * net_in_grad[-2]
        x_vector = make_onehot(x[-1], self.vocab_size)
        sum_deltaV = np.outer(delta_in_t, x_vector)

        if len(s)>2:
            sum_deltaU = np.outer(delta_in_t, s[-3])
        else:
            sum_deltaU = np.outer(delta_in_t, s[-2])
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		no return values
		'''
        net_out_gradbptt = np.ones(len(x))
        net_in_gradbptt = np.array([s_t * (np.ones(len(s_t)) - s_t) for s_t in s])
        sum_deltaWbptt = np.zeros((self.vocab_size, self.hidden_dims))
        sum_deltaVbptt = np.zeros((self.hidden_dims, self.vocab_size))
        sum_deltaUbptt = np.zeros((self.hidden_dims, self.hidden_dims))

        # NOTE: in this implement, s[0] = [0. 0. ... 0.]
        #       and s[t+1] is the hidden layer corresponding to y[t] (same in net_in_grad)
        for t in reversed(range(len(x))):
            d_t_vector = make_onehot(d[t], self.out_vocab_size)
            delta_out_t = (d_t_vector - y[t]) * net_out_gradbptt[t]
            sum_deltaWbptt += np.outer(delta_out_t, s[t])
            ...
            for tau in range(0, 1 + steps):
                if t - tau < 0:
                    break
                if tau == 0:
                    delta_inbptt[t - tau] = np.dot(self.W.T, delta_out_t) * net_in_gradbptt[t]
                else:
                    delta_inbptt[t - tau] = np.dot(self.U.T, delta_inbptt[t - tau + 1]) * net_in_gradbptt[t - tau]
                x_vector = make_onehot(x[t - tau], self.vocab_size)
                sum_deltaVbptt += np.outer(delta_inbptt[t - tau], x_vector)
                sum_deltaUbptt += np.outer(delta_inbptt[t - tau], s[t - tau - 1])

        # multiply learning rate when actually applying delta
        self.deltaW += sum_deltaWbptt
        self.deltaV += sum_deltaVbptt
        self.deltaU += sum_deltaUbptt
    ...
    def acc_deltas_bptt_np(self, x, d, y, s, steps):
        ...
        d_t_vector = make_onehot(d[-1], self.out_vocab_size)
        delta_out_t = (d_t_vector - y[-1]) * net_out_gradbptt[-1]
        sum_deltaWbptt = np.outer(delta_out_t, s[-2])
    ...
    def compute_loss(self, x, d):
        ...
        for t in range(len(y)):
            ...
            loss_t[t] = -np.dot(d_t_vector, np.log(y_t_vector))
    ...
    def compute_loss_np(self, x, d):
        ...
        y, s = self.predict(x)
        d_vector = make_onehot(d[0], self.out_vocab_size)
        y_vector = y[-1]
        loss = -np.dot(d_vector, np.log(y_vector))
    ...
    def train_np(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100,
        ...
        for epoch in range(epochs):
            ...
            loss = sum([loss_function(X_dev[i], D_dev[i]) for i in range(len(X_dev))]) / loss_sum
            acc = sum([self.compute_acc_np(X_dev[i], D_dev[i]) for i in range(len(X_dev))]) / float(len(X_dev))
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        # load the pre-trained matrics

        # UU = np.load('rnn.U.npy')
        # VV = np.load('rnn.V.npy')
        # WW = np.load('rnn.W.npy')
        ...
        for lr in [0.5, 0.1, 0.05]:
            for lb in [0,2,5]:
                for hd in [25,50]:
                    print('hidden dim:{0}, look back:{1}, learning rate:{2}'.format(hd,lb,lr))

                    train_RNN = RNN(vocab_size, hd, vocab_size)
                    run_loss = train_RNN.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev,
							 epochs=10, learning_rate=lr, back_steps=lb)
        ...
        train_RNN = RNN(vocab_size, 25, vocab_size)
        #train_RNN.set_matric(UU, VV, WW)

        run_loss = train_RNN.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev,
                                               epochs=15, learning_rate=0.5,
                                               back_steps=2)

        # test_loss = sum([train_RNN.compute_loss(X_test[i], D_test[i]) for i in range(len(X_test))]) / sum([len(d) for d in D_test])
        ...
        for lr in [0.5, 0.1, 0.05]:
            for lb in [0, 2, 5]:
                for hd in [25, 50]:
                    ...
                    train_RNN = RNN(vocab_size, hd, 2)
                    run_loss, run_acc = train_RNN.train_np(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev,
                                               epochs=20, learning_rate=lr, back_steps=lb)

                    print("Accuracy: %.03f" % run_acc)
        """


        train_RNN = RNN(vocab_size, 50, 2)
        train_RNN.set_matric(UU, VV, WW)


        test_loss = sum([train_RNN.compute_loss_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(D_test)
        test_acc = sum([train_RNN.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))])/ len(D_test)

        # run_loss, run_acc = train_RNN.train_np( X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev,
        #                                         epochs=20, learning_rate=0.5, back_steps=0)


        # np.save('np.U.npy',train_RNN.U)
        # np.save('np.V.npy', train_RNN.V)
        # np.save('np.W.npy', train_RNN.W)


        print("Loss: %.03f" % test_loss)
        print("Accuracy: %.03f" % test_acc)
    ...
    if mode == "analysis":
        ...
        # load development data
        sents = load_np_dataset(data_folder + '/wiki-dev.txt')
        dev_back_size = load_back_dataset(data_folder + '/wiki-dev.txt')
        S_dev = docs_to_indices(sents, word_to_num, 0, 0)
        X_dev, D_dev = seqs_to_npXY(S_dev)

        # plt.hist(dev_back_size, max(dev_back_size), normed=0, facecolor='g', )
        # plt.xlabel('distance between subject and verb')
        # plt.ylabel('frequencise')
        ...
        r = RNN(vocab_size, 50, 2)
        r.set_matric(UU, VV, WW)

        acc = np.zeros(max(dev_back_size))

        for b in range(max(dev_back_size)):
            X_back = [X_dev[i] for i in range(len(X_dev)) if dev_back_size[i] == b]
            D_back = [D_dev[i] for i in range(len(D_dev)) if dev_back_size[i] == b]
            if len(X_back) != 0:
                acc[b]=sum([r.compute_acc_np(X_back[i], D_back[i]) for i in range(len(X_back))]) / len(X_back)
                tmp = acc[b]
            else:
                acc[b] = tmp

        acc_5 = np.zeros(max(dev_back_size))
        UU = np.load('np5.U.npy')
        VV = np.load('np5.V.npy')
        WW = np.load('np5.W.npy')
        ...
        for b in range(max(dev_back_size)):
            ...
            D_back = [D_dev[i] for i in range(len(D_dev)) if dev_back_size[i] == b]
            if len(X_back) != 0:
                acc_5[b]=sum([r.compute_acc_np(X_back[i], D_back[i]) for i in range(len(X_back))]) / len(X_back)
                tmp = acc_5[b]
            else:
                acc_5[b] = tmp

        acc_2 = np.zeros(max(dev_back_size))
        UU = np.load('np2.U.npy')
        VV = np.load('np2.V.npy')
        WW = np.load('np2.W.npy')
        ...
        for b in range(max(dev_back_size)):
            ...
            D_back = [D_dev[i] for i in range(len(D_dev)) if dev_back_size[i] == b]
            if len(X_back) != 0:
                acc_2[b]=sum([r.compute_acc_np(X_back[i], D_back[i]) for i in range(len(X_back))]) / len(X_back)
                tmp = acc_2[b]
            else:
                acc_2[b] = tmp

        acc_10 = np.zeros(max(dev_back_size))
        UU = np.load('np10.U.npy')
        VV = np.load('np10.V.npy')
        WW = np.load('np10.W.npy')
        ...
        for b in range(max(dev_back_size)):
            ...
            D_back = [D_dev[i] for i in range(len(D_dev)) if dev_back_size[i] == b]
            if len(X_back) != 0:
                acc_10[b] = sum([r.compute_acc_np(X_back[i], D_back[i]) for i in range(len(X_back))]) / len(X_back)
                tmp = acc_10[b]
            else:
                acc_10[b] = tmp

        # accuracy = [0.708, 0.750, 0.733, 0.783, 0.801, 0.812, 0.818, 0.794, 0.828, 0.823, 0.831,0.833,0.795,0.822,0.831,0.837,0.837,0.841,0.843,0.836]
        #
        # loss = [4.7935,4.7197,4.6683,4.6306,4.5710,4.5325,4.5097,4.4895,4.4876,4.4605,4.4499,4.4595,4.4240,4.4182, 4.4290]
        #
        # x = np.linspace(1, 20, 20)


        plt.plot(range(len(acc)), acc, color='green', linestyle='--', label='look back = 0' )
        plt.plot(range(len(acc_2)), acc_2, color='cornflowerblue', linestyle='--', label='look back = 2' )
        plt.plot(range(len(acc_5)), acc_5, color='orange', linestyle='--', label='look back = 5')
        plt.plot(range(len(acc_10)), acc_10, color='brown', linestyle='--', label='look back = 10')

        # plt.plot(x, accuracy, color='green', linestyle='--')
        # for i in range(len(accuracy)):
        #     plt.text(i, accuracy[i], accuracy[i], ha='center', va='bottom', fontsize=9)

        plt.xlabel('epoch')
        plt.ylabel('loss')
        plt.legend(loc=0)
        plt.savefig('fig/q3acc.pdf')
        plt.show()
